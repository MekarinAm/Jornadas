<?php
    ...
    if(isset($_SESSION['identificadorUsuario']))
    {
        ...
        $monto = 300.00;
        $concepto = 'Completo'; //nombre del paquete
        ...
        $estatus = "No pagado";
        $cred = 1.5;
        if($numpag<120){/**verificará si el número de registros del paquete es menor a 120 */
            /**asignará el folio correspondiente*/
            $idd = str_replace("PC","",$numc);/*continuará con la numeración automática del folio*/
            $id = str_pad($numc,8,STR_PAD_LEFT);/**agregará 6 digitos más a la derecha para el folio (número de control) */
            $folio = 'PC'.$id;/**guardará el folio en la variable */
